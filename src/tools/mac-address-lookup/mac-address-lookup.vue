...
const { copy } = useCopy({ source: () => details.value ?? '' });
</script>
...
<template>
  <div>
    <c-input-text
      ...
      :label="t('tools.mac-address-lookup.inputLabel')"
      size="large"
      :placeholder="t('tools.mac-address-lookup.inputPlaceholder')"
      clearable
      autocomplete="off"
      autocorrect="off"
      autocapitalize="off"
    ...
    <div mb-5px>
      {{ t('tools.mac-address-lookup.vendorInfoLabel') }}
    </div>
    <c-card mb-5>
      <div v-if="details">
    ...
    <div flex justify-center>
      <c-button :disabled="!details" @click="copy(undefined, { notificationMessage: t('tools.mac-address-lookup.copied') })">
        {{ t('tools.mac-address-lookup.copy') }}
      </c-button>
    </div>
  </div>
</template>
