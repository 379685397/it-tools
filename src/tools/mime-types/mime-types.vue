...
<template>
  <c-card>
    ...
    <div style="opacity: 0.8">
      {{ t('tools.mime-types.mimeToExtDescription') }}
    </div>
    <c-select
    ...
    <div v-if="extensionsFound.length > 0">
      {{ t('tools.mime-types.extensionsForMimeTypePrefix') }} <n-tag round :bordered="false">
        {{ selectedMimeType }}
      </n-tag> {{ t('tools.mime-types.extensionsForMimeTypeSuffix') }}
  ...
  <c-card>
    ...
    <div style="opacity: 0.8">
      {{ t('tools.mime-types.extToMimeDescription') }}
    </div>
    <c-select
      ...
      :placeholder="t('tools.mime-types.selectExtensionPlaceholder')"
    />

    <div v-if="selectedExtension">
      {{ t('tools.mime-types.mimeForExtensionPrefix') }} <n-tag round :bordered="false">
        {{ selectedExtension }}
      </n-tag> {{ t('tools.mime-types.mimeForExtensionSuffix') }}
      <div style="margin-top: 10px">
  ...
  <div>
    <n-table>
      <thead>
        <tr>
          <th>{{ t('tools.mime-types.tableMimeTypes') }}</th>
          <th>{{ t('tools.mime-types.tableExtensions') }}</th>
        </tr>
      </thead>
      <tbody>
