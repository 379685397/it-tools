...
const encodingOptions = computed(() => [
  { label: t('tools.hmac-generator.encoding.bin'), value: 'Bin' },
  { label: t('tools.hmac-generator.encoding.hex'), value: 'Hex' },
  { label: t('tools.hmac-generator.encoding.base64'), value: 'Base64' },
  { label: t('tools.hmac-generator.encoding.base64url'), value: 'Base64url' },
]);
</script>
...
<template>
  <div flex flex-col gap-4>
    ...
    <div flex gap-2>
      <c-select
        v-model:value="hashFunction" :label="t('tools.hmac-generator.hashFunctionLabel')"
        flex-1
        :placeholder="t('tools.hmac-generator.hashFunctionPlaceholder')"
        :options="Object.keys(algos).map((label) => ({ label, value: label }))"
      />
      <c-select
        v-model:value="encoding" :label="t('tools.hmac-generator.outputEncodingLabel')"
        flex-1
    ...
    <div flex justify-center>
      <c-button @click="copy(undefined, { notificationMessage: t('tools.hmac-generator.copied') })">
