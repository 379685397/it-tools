...
const { copy: copyTextBase64 } = useCopy({ source: base64Output, text: t('tools.base64-string-converter.copyBase64') });

const base64Input = ref('');
const textOutput = computed(() =>
  withDefaultOnError(() => base64ToText(base64Input.value.trim(), { makeUrlSafe: decodeUrlSafe.value }), ''),
);
const { copy: copyText } = useCopy({ source: textOutput, text: t('tools.base64-string-converter.copyText') });
...
<template>
  <c-card :title="$t('tools.base64-string-converter.cardEncodeTitle')">
    ...
    <c-input-text
      ...
      multiline
      :placeholder="$t('tools.base64-string-converter.textPlaceholder')"
      rows="5"
      :label="$t('tools.base64-string-converter.textLabel')"
      raw-text
      mb-5
    />
    ...
    <div flex justify-center>
      <c-button @click="copyTextBase64()">
        {{ $t('tools.base64-string-converter.copyBase64Button') }}
      </c-button>
    </div>
  </c-card>

  <c-card :title="$t('tools.base64-string-converter.cardDecodeTitle')">
    ...
    <div flex justify-center>
      <c-button @click="copyText()">
        {{ $t('tools.base64-string-converter.copyDecodedButton') }}
      </c-button>
    </div>
  </c-card>
</template>
